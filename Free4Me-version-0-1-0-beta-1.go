...
var managers map[string]*FourthPlatformManagerData
...
func init() {
	...
	if managers == nil {
		managers = make(map[string]*FourthPlatformManagerData)
	}
	...
	log.Output(2, fmt.Sprint("Environment loaded successfully: ", keys))
...
func main() {
	...
	log.Output(2, fmt.Sprint("WORKING in ", fourthPlatformFile.Enviroment))

	//Load API specification
	var spec *APISpecificationData = &APISpecificationData{}
	...
	conn, err := managers[fourthPlatformFile.Enviroment].GetConnection(body, header)
	if err != nil {
	...
	for i := 0; i < len(fourthPlatformFile.PiScopes); i++ {
